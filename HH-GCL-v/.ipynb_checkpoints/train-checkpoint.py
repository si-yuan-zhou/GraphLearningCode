...
from util import label_classification, get_dataset, build_hypergrah, build_hypergrah_I


def train(model: Model, x, edge_index, V, E):
    model.train()
    optimizer.zero_grad()

    z1, z2 = model(x, edge_index)
    loss = model.lossI(z1, z2, batch_size=0)    # node-node only
    # loss = model.lossIII(z1, z2, V, E, batch_size=0)   # node-node & node-subgraph & sungraph-graph
...
def test(model: Model, x, edge_index, y):
    ...
    print("---------- fusion graph node result ---------------")
...
if __name__ == '__main__':
    ...
    # args, V, E = build_hypergrah_I(args, dataset[0])
    ...
    for epoch in range(1, num_epochs + 1):
        loss = train(model, data.x, data.edge_index, V, E)
